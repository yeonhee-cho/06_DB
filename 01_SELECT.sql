...
작성법 -2
- 테이블 내 모든 컬럼을 선택해서 모든 행과 모든 컬럼을 조회
SELECT *
FROM 테이블명;
*/
...
# Employees 테이블에서 사번, 이름, 급여 조회(emp_id, full_name, salary)
...
# Training_programs 테이블에서 프로그램명, 교육시간 조회(program_name, duration_hours)
SELECT program_name, duration_hours 
FROM training_programs;

/**************
컬럼 값 산술 연산자
-- 컬럼 값 : 행과 열이 교차되는 테이블의 한 칸에 작성된 값alter
...
-- Employee 테이블에서 이름, 급여, 급여 * 0.8(세후급여) 조회
...
-- Departments 테이블에서 부서명, 예산(budget), 예산 * 1.1        예산 + 10%(예산 * 1.1)의 총액 조회
SELECT dept_name, budget, budget * 1.1
FROM departments;

-- 모든 SQL에서는 DUAL 가상 테이블이 존재함. MySQL에서는 FROM을 생략할 경우 자동으로 DUAL 가상 테이블을 사용
-- (가상 테이블 필요없음)
-- 현재 날짜 확인하기
...
``(백틱) 이나 ""(큰따옴표)나 ''(작은따옴표) 사용가능
대/소문자 구분
(표준으로는 `백틱`이 가장 많이 쓰이고, '작은따옴표'는 문자열 값으로만 사용되어 `백틱`과 "큰따옴표"가 핵심)
**********************/
...
-- 1. employees 테이블에서 사번, 이름, 이메일로 해당 컬럼 데이터 조회 (별칭에서 as `` 사용하지 않고 조회)
SELECT emp_code AS 사번, full_name AS 이름, email AS 이메일 
FROM employees;

-- 2. employees 테이블에서 이름, 급여, 연봉(급여*12)로 해당 컬럼 데이터 조회 (별칭에서 as `` 사용하고 조회)
SELECT full_name AS `이름`, salary AS `급여`, ceil(salary * 12) AS `연봉(급여*12)`
FROM employees;

-- 3. positions 테이블에서 직급명, 최소급여, 최대급여, 급여차이 명칭으로 해당 컬럼 데이터 조회 (별칭에서 as `` 사용하고 조회)
...
-- 조회한 결과가 존재하지 않는다, 조회 결과가 0이 나온 순간
-- 에러가 아님!!
...
-- 02으로 시작하는 사원의 이름, 전화번호 조회
SELECT full_name, phone
FROM employees
WHERE phone LIKE '02%';

-- EMPLOYEES 테이블에서 email의 아이디((@)기준)가 
-- 3글자인 사원의 이름, 이메일 조회
...
-- EMPLOYEES 테이블에서 급여가 4000만 이상, 
-- 7000만 이하인 사원의 사번, 이름, 급여 조회
-- AND BETWEEN 구문을 이용한 두가지 SQL문 작성하기
SELECT emp_id, full_name, salary
FROM employees
...
-- EMPLOYEES 테이블에서 급여가 4000만 미만 또는
-- 8000만 초과인 사원의 사번, 이름, 급여 조회
-- emp_id, full_name, salary
-- OR NOT BETWEEN 구문을 이용한 두가지 SQL문 작성하기
SELECT emp_id, full_name, salary
FROM employees
WHERE salary < '40000000' 
OR salary > '80000000';
...
-- EMPLOYEES 테이블에서 부서 ID가 4인 사원 중
-- 급여가 4000만 이상, 7000만 이하인 사원의 사번, 이름, 급여 조회
-- AND BETWEEN 구문을 이용한 두가지 SQL문 작성하기
SELECT emp_id, full_name, salary
FROM employees
WHERE dept_id = 4 AND 
(salary BETWEEN '40000000' AND '70000000');
...
JOIN은 테이블 간의 기준 컬럼을 통해 하나의 결과를 조회할 때 사용
JOIN을 이용해서 VIEW 테이블을 생성한 후 VIEW 테이블에서 데이터를 읽어오는 것이 효율적임!!
***********************/
/*
USE employee_management;
다른 데이터베이스에서 잠시 조회하고자하는 테이블을 조회할 때는 
SELECT * FROM 다른데이터베이스명칭.테이블명칭; 으로 조회가 가능!
SELECT * FROM employee_management.employees;

-- USE 하지 않고도 SELECT * FROM employee_management.employees; 이렇게 사용 가능
*/
