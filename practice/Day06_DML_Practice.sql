...
INSERT INTO customers VALUES(NULL, '조민수', 'minsoo.jo@gmail.com', 'min123!', '010-2468-1357', '대전시 유성구 대학로 99', now());

-- 문제 2
-- CATEGORIES 테이블에 새로운 카테고리들을 한 번에 추가하시오.
-- Vietnamese (베트남 요리), Western (양식), Salad (샐러드)
SELECT * FROM categories;
INSERT INTO categories VALUES(NULL, 'Vietnamese'),
							 (NULL, 'Western'),
						     (NULL, 'Salad');
...
SELECT * FROM stores;
INSERT INTO stores VALUES(NULL, '맘스터치 대학로점', '9', '대전시 유성구 대학로 100', '042-123-4567', '수제버거 전문점', '12000', '2500', '4.6', '매일 10:30 - 22:00', now());
...
-- TODO SUBQUERY 이용해서 한 번에 작성하기
-- 가게에서 CHICKEN 카테고리에 속한 모든 매장의 운영 시간을 매일 12시간으로 변경
-- 1. SELECT * FROM categories WHERE category_name = 'Chicken';
SELECT * FROM categories WHERE category_name = 'Chicken';

-- 매일 12시간과 Chicken은 html에서 소비자가 변경하고자하여 선택하거나 작성한 데이터를 javaScript 변수 이름을 통해서
-- java 서버로 전달되고, java에서는 javaScript로 전달받은 데이터를 java 내부에 설정한 변수 이름으로 전달 받은 다음에 
...
-- ... TODO
/*
SELECT * FROM categories WHERE category_name = 'Korean'; -- 2
SELECT * FROM stores;

SELECT * FROM stores s, categories c
WHERE s.category_id = c.category_id 
AND c.category_id = (SELECT category_id FROM categories WHERE category_name = 'Korean');
...
-- TODO SUBQUERY 이용해서 가장 최근에 주문한 ID를 조회하고 상태 변경
...
SELECT * FROM stores WHERE store_name = '동대문엽기떡볶이 신림점'; -- category_id 6
SELECT * FROM menus WHERE store_id = 6; 
UPDATE menus 
SET price = ROUND(price * 1.1) 
WHERE store_id = 6;

-- menu_id 12인 price = 14000
UPDATE menus
SET price = 14000
WHERE menu_id = 12;
-- menu_id 14인 price = 2000
UPDATE menus
SET price = 2000
WHERE menu_id = 14;
-- UPDATE를 이용해서 14000과 2000원으로 원상복구

-- SELECT * FROM stores WHERE store_name = '동대문엽기떡볶이 신림점'; -- store_id 조회하고 모든 메누 가격 1.1 인상
-- UPDATE SUBQUERY 이용해서 동대문엽기떡볶이 신림점 조회한 후 모든 메뉴 가격 10% 인상
UPDATE menus
SET price = price * 1.1
...
SET phone = '미등록' 
WHERE phone IS NULL;
-- ERROR 1062 : UNOQUE 는 NULL 값도 모든 컬럼에서 1개만 존재해야 함
-- 빈 칸 마저도 특별하게 고유하게 단일로 존재해야하는 데이터!
-- Error Code: 1062. Duplicate entry '미등록' for key 'customers.phone'	0.016 sec
...
SELECT * FROM menus WHERE price >= 20000;
UPDATE menus
SET is_popular = true
WHERE price >= 20000;
